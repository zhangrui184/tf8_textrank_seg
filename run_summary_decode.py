...
def main(unused_argv):
  ...
  if hps.mode == 'train':
    ...
    decoder = BeamSearchDecoder(model, batcher, vocab,decode_model_hps)
    #vsize = self._vocab.size()
    #vsize=hps.vocab_size
    my_embedding = tf.get_variable('embedding', [50000, hps.emb_dim], dtype=tf.float32)
    decoder.decode(my_embedding=my_embedding) # decode indefinitely (unless single_pass=True, in which case deocde the dataset exactly once)
  else:
